...
#define SETTINGS_VERSION_LEGACY_US 1u
...
static uint32_t settings_checksum(uint32_t version, uint32_t pulse_time_ns, uint32_t pulse_power_bits) {
    return SETTINGS_MAGIC ^ version ^ pulse_time_ns ^ pulse_power_bits ^ 0xC0DEC0DEu;
}

static bool settings_validate(const settings_storage_t *storage) {
    float_u32_t pulse_power = {.u32 = storage->pulse_power_bits};

    if(storage->magic != SETTINGS_MAGIC) {
        return false;
    }

    if(storage->version != SETTINGS_VERSION_LEGACY_US && storage->version != SETTINGS_VERSION) {
        return false;
    }

    if(storage->checksum != settings_checksum(storage->version, storage->pulse_time_ns, storage->pulse_power_bits)) {
        return false;
    }
...
void picoemp_settings_load_defaults(picoemp_settings_t *settings) {
    ...
    settings->pulse_time_ns = PICOEMP_DEFAULT_PULSE_TIME_NS;
...
bool picoemp_settings_load(picoemp_settings_t *settings) {
    ...
    pulse_time_ns = storage->pulse_time_ns;
    if(storage->version == SETTINGS_VERSION_LEGACY_US) {
        pulse_time_ns *= 1000u;
        if(pulse_time_ns > UINT32_MAX) {
            pulse_time_ns = UINT32_MAX;
        }
    }

    settings->pulse_time_ns = (uint32_t)pulse_time_ns;
...
bool picoemp_settings_save(const picoemp_settings_t *settings) {
    ...
    if(settings_validate(current) &&
       ...
       current->pulse_time_ns == settings->pulse_time_ns &&
    ...
    storage->pulse_time_ns = settings->pulse_time_ns;
    storage->pulse_power_bits = pulse_power.u32;
    storage->checksum = settings_checksum(storage->version, storage->pulse_time_ns, storage->pulse_power_bits);
